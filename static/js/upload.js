...
async function uploadFile(file) {
    ...
    return new Promise((resolve, reject) => {
        ...
        // Обработчик события для состояния запроса
        http.onreadystatechange = function () {
            if (http.readyState === XMLHttpRequest.DONE) {
                if (http.status === 200) {
                    // Если запрос успешен, резолвим промис
                    console.log(http.status, http.statusText)
                    resolve(http.responseText);
                } else {
                    // Если произошла ошибка, реджектим промис
                    reject(new Error(`Ошибка при отправке файла: ${http.status} ${http.statusText}`));
                }
            }
        }
...
// Используем Promise.all, чтобы отследить завершение всех промисов
async function finishPromises(promises) {
    Promise.all(promises)
    .then(responses => {
        reload_page_without_cache(2000)
    })
    .catch(error => {
        console.error("Произошла ошибка при отправке одного или нескольких файлов:", error)
    })
}
